...
def load_assets():
    ...
    return model, scaler, feature_cols, fraud_samples, legit_samples

model, scaler, feature_cols, fraud_samples, legit_samples = load_assets()
...
def generate_live_data(n=15):
    ...
    batch_legit = legit_samples.sample(n_legit)

    data = pd.concat([batch_fraud, batch_legit]).sample(frac=1).reset_index(drop=True)
    return data
